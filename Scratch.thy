...
(*type_synonym ('a, 's) Weighted = "('a \<times> 's) list"*)
...
lift_definition scaleWeighted :: "'s::ssr \<Rightarrow> ('a, 's) FWeighted \<Rightarrow> ('a, 's) FWeighted" is
...
definition returnWeighted :: "('a, 's::ssr) GraphOfFWeighted" where
  "returnWeighted x = fmap_of_list [(x, one)]"
...
context includes fmap.lifting begin
fun bindWeighted :: "('a, 's::ssr) FWeighted \<Rightarrow> ('a \<Rightarrow> ('a, 's) FWeighted) \<Rightarrow> ('a, 's) FWeighted" where
  "bindWeighted fmempty k = fmempty" |
  "bindWeighted (xs ++\<^sub>f ys) k = (scaleWeighted p (k x)) ++\<^sub>f (bindWeighted xs k)"
end
